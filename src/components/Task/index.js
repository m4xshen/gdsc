...
export default function Task({ initTask, initSubtasks, onClick }) {
  ...
  useEffect(() => {
    ...
    labels.forEach(label => {
      if (!label.querySelector('svg')) { // Check if the label already contains an SVG
        ...
        label.insertBefore(svgIcon, label.firstChild);
      }
    });
  }, [task.labels]); // Run this effect whenever the task labels change
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.subtasksAndDetail}>
        <div className={styles.subTasks}>
          {subtasks.map(subtask => (
            <Task key={subtask.id} initTask={subtask} initSubtasks={subtask.subtasks} />
          ))}
        </div>
